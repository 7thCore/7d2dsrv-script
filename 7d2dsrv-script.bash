...
#7 Days to Die server script by 7thCore
...
#SteamCmd variables
APPID="294420" #Steam app id for the server
...
script_add_server() {
	...
	if [[ "$ADD_SERVER_INSTANCE_ADD" =~ ^([yY][eE][sS]|[yY])$ ]]; then
		...
		if [ -f "$SRV_DIR/$SERVER_INSTANCE_ADD/7DaysToDieServer.x86_64" ]; then
...
script_poststop() {
	...
	if [ -f "/tmp/$SERVICE_NAME-$1-telnet-tmux.log" ]; then
		rm /tmp/$SERVICE_NAME-$1-tmux.log
	fi

	if [ -f "/tmp/$SERVICE_NAME-$1-telnet-tmux.conf" ]; then
		rm /tmp/$SERVICE_NAME-$1-tmux.conf
	fi
...
script_save() {
	...
	for SERVER_SERVICE in $(systemctl --user list-units -all --no-legend --no-pager --plain $SERVICE_NAME@*.service $SERVICE_NAME-tmpfs@*.service | awk '{print $1}' | tr "\\n" "," | sed 's/,$//'); do
		...
		if [[ "$(systemctl --user show -p ActiveState --value $SERVER_SERVICE)" == "active" ]] && [[ "$(systemctl --user show -p UnitFileState --value $SERVER_SERVICE)" == "enabled" ]]; then
			...
			( sleep 5 && tmux -L $SERVICE_NAME-$SERVER_INSTANCE-telnet-tmux.sock send-keys -t $NAME.0 'saveworld' ENTER ) &
			...
			done < <(tail -n1 -f /tmp/$SERVICE_NAME-$SERVER_INSTANCE-telnet-tmux.log)'
			EXIT_CODE="$?"
...
script_server_tmux_install() {
	...
	if [ -z "$2" ]; then
		echo "$(date +"%Y-%m-%d %H:%M:%S") [$VERSION] [$NAME] (Server tmux configuration) Installing tmux telnet configuration for server $1." | tee -a "$LOG_SCRIPT"
		TMUX_CONFIG_FILE="/tmp/$SERVICE_NAME-$1-telnet-tmux.conf"
	elif [[ "$2" == "override" ]]; then
		echo "$(date +"%Y-%m-%d %H:%M:%S") [$VERSION] [$NAME] (Server tmux configuration) Installing tmux telnet override configuration for server $1." | tee -a "$LOG_SCRIPT"
		TMUX_CONFIG_FILE="$CONFIG_DIR/$SERVICE_NAME-$1-telnet-tmux.conf"
	fi

	if [ -f $CONFIG_DIR/$SERVICE_NAME-$1-telnet-tmux.conf ]; then
		cp $CONFIG_DIR/$SERVICE_NAME-$1-telnet-tmux.conf /tmp/$SERVICE_NAME-$1-telnet-tmux.conf
	else
		if [ ! -f $TMUX_CONFIG_FILE ]; then
			touch $TMUX_CONFIG_FILE
			cat > $TMUX_CONFIG_FILE <<- EOF
			...
			set-hook -g session-created 'resize-window -y 24 -x 10000'
			set-hook -g session-created "pipe-pane -o 'tee >> /tmp/$SERVICE_NAME-$1-telnet-tmux.log'"
			set-hook -g client-attached 'resize-window -y 24 -x 10000'
			set-hook -g client-detached 'resize-window -y 24 -x 10000'
			set-hook -g client-resized 'resize-window -y 24 -x 10000'
			...
			#Ctrl-b [ (in copy mode you can navigate the buffer including scrolling the history. Use vi or emacs-style key bindings in copy mode. The default is emacs. To exit copy mode use one of the following keybindings: vi q emacs Esc)
			EOF
			echo "$(date +"%Y-%m-%d %H:%M:%S") [$VERSION] [$NAME] (Server tmux configuration) Tmux telnet configuration for server $1 installed successfully." | tee -a "$LOG_SCRIPT"
		fi
	fi
}
